...
class _MyLoansTabState extends State<MyLoansTab> {
  ...
  Widget build(BuildContext context) {
    return new Scaffold(
      appBar: AppBar(
          title: const Text('Objetos Prestados'),
          //TODO: searchBar
      ),
      body: FutureBuilder<List<Obj>>(
        future: widget._group.getLoansMeToOthers(),
          ...
          children: [
            GestureDetector(
              onTap: () {
                showDialog(
                  context: context,
                  ///Por ahora solo entre miembros de grupo.
                  builder: (BuildContext context){
                      return UserDetails(lentObject.groupObjectState.actualUser);
                  }
                );
              },
              child: Text(lentObject.groupObjectState.actualUser.name) //WIP: hacerlo más clickable
            ),
            Text(' tiene '),
